...
# First start by defining what is meant by 'similarity' - is it semantic, syntactic or word size?
# Cats and monkeys are both animals so are closer than cat and banana, also monkeys are associated with bananas
# This would suggest semantic links

print(word1.similarity(word2))
print(word3.similarity(word2))
print(word3.similarity(word1))

# My own example - interestingly, while monkeys and apes are shown as closely linked,
# humans and apes are not. Is this a bias towards our ourselves?
tokens = nlp("cat human monkey ape")
...
# The same comparisons but using the simpler language model "en_core_web_sm"
print("-------------Complaints similarity with simpler model---------------")
for token in complaints:
    token = nlp_sm(token)
    for token_ in complaints:
...
# The same comparisons but using the simpler language model "en_core_web_sm"
print("-------------Recipes similarity with simpler model---------------")
for token in recipes:
    token = nlp_sm(token)
    for token_ in recipes:
        token_ = nlp_sm(token_)
...
# The same comparisons but using the simpler language model "en_core_web_sm"
print("-------------Recipes and complaints similarity with simpler model---------------")
for token in recipes:
    token = nlp_sm(token)
    for token_ in complaints:
        token_ = nlp_sm(token_)
...
"""The similarity index has significantly reduced from what was observed with the advanced language model. 
There is a warning in the console that explains 'The model you're using has no word vectors loaded, so the result of 
the Doc.similarity method will be based on the tagger, parser and NER, which may not give useful similarity 
judgements.' 'en_core_web_sm' is a small model which does not come with word vectors."""
